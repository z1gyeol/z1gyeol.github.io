...
3. 계산
+ 다 끝났고 부피 계산만이 남았다. 문제에 부피 구하는 방법은 나와있다. 그러므로 필요한 것은 다각형의 면적과 중심이다. 넓이는 그 유명한 신발끈 공식을 쓰면 되니 중심을 구해보자.<br>
	 $C_x={1\over6A}\sum\limits^{n-1}_{k=0}(x_i+x_{i+1})(x_iy_{i+1}-x_{i+1}y_i)$<br>
	 $C_x={1\over6A}\sum\limits^{n-1}_{k=0}(y_i+y_{i+1})(x_iy_{i+1}-x_{i+1}y_i)$<br>
	 여기서 $A$는 넓이이다. 그리고 $i+1$에서 꼭 $(i+1)\%n$을 쓰자.
	 부피는 다음과 같다.<br>
	 $V=2\pi A\overline x$<br>
